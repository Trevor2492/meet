...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if ( !navigator.onLine ){
      this.setState({
        infoText: 'You are currently offline and any events shown may not be accurate. Go online then re-open the app to load current events.'
      });
    }
  ...
  getData = () => {
    ...
    const data = locations.map(location => {
      const number = events.filter(event => event.location === location).length //This line gets the number of events in each city location
